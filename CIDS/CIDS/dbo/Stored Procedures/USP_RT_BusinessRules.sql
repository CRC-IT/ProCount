...
BEGIN TRY
	...
	BEGIN
		...
		-- START - BUSINESS RULE 2 - Last_Entity_Name must not be longer than 200 characters
			...
			AND ((TICD.LocAccountNumber1 = ' ' OR TICD.LocAccountNumber1 IS NULL) 
			OR (TICD.locationid IS NULL OR TICD.locationid = ' ') 
			OR (TICD.BusinessEnitity  IS NULL OR TICD.BusinessEnitity  = ' ')
			OR (TICD.LocAccountNumber2  IS NULL OR TICD.LocAccountNumber2  = ' ')
			OR (TICD.AreaID  IS NULL OR TICD.AreaID  = ' ')
			)
			...
			BEGIN
				...
				FROM [dbo].[tbl_MsgQueue] TMQ	INNER JOIN [dbo].[tbl_IFace_TankRunTicket] TICD	ON TMQ.TransID = TICD.TransID AND TMQ.TransSeq = TICD.TransSeq AND TMQ.IFaceBatchUID = TICD.IFaceBatchUID
				LEFT JOIN [dbo].[tbl_SubscriberController] TSC	ON TMQ.SubID = TSC.SubID 
				WHERE TMQ.SubIFace = 'TankRunTicket' AND TSC.SubscriberName = 'ONECALPS'	--AND TICD.RunHours  between 0 and 24
	...
	BEGIN
		...
		(
			...
			FROM @TBL_ERRORS 
			FOR XML PATH ('ErrorMessage'), ROOT('BR_RT_ERRORS')
		)
		...
		VALUES ( 'USP_RT_BusinessRules','StoredProcedure','TankRunTicket', @Destination + ' BusinessRule Validations','Error while executing the BusinessRules on TankRunTicket Data', @XmlData, GETDATE(),HOST_NAME(), CURRENT_USER)
